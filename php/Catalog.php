    ...
    private function Ajax()
    {
        if(filter_input(INPUT_POST, "opcion")!=NULL and filter_input(INPUT_POST, "opcion")!=FALSE){
            ...
            switch (filter_input(INPUT_POST, "opcion"))
            {
                ...
                case 'D_Catalog': $this->D_Catalog(); break;
                case 'D_Column': $this->D_Column(); break;
            }
        }
    }
...
//        if($TipoCatalogo=='ListSearch'){$ListSearch=' LIMIT 25';}
    ...
    public function deleteCatalog($dataBaseName, $idRepository, $repositoryName, $catalogName){
        ...
        if(($queryResult = $DB->ConsultaQuery($dataBaseName, $query))!=1)
            return "Error al intentar elimina
            r el catálogo $catalogName. $queryResult";
    ...
    public function D_Catalog()
    {
        header('Content-type: application/json');
        try{
            $DB = new DataBase();
            $name = filter_input(INPUT_POST, "NameCatalog");
            $nameDB = filter_input(INPUT_POST, "dataBaseName");
            $RepositoryName = filter_input(INPUT_POST, "RepositoryName");

            if (isset($name) && isset($RepositoryName)){
                $query = "DELETE FROM `CSDocs_Catalogos` WHERE `CSDocs_Catalogos`.`NombreCatalogo` = '$name'";
                $queryResult = $DB->ConsultaQuery($nameDB, $query);

                if ($queryResult == 1){
                    $queryDeleteCat = "DROP TABLE $RepositoryName"."_$name";
                    $resultDelete = $DB->ConsultaQuery($nameDB, $queryDeleteCat);
                    DesignerForms::DeleteStructure($nameDB, $RepositoryName."_$name");

                    if ($resultDelete == 1){
                        $response = [
                            "status" => true ,
                            "message" => "El catálago $name se elimino correctamente."
                        ];
                ...
                $response = [
                    "status" => false ,
                    "message" => "Algo salio mal al tratar de eliminar el catálago."
                ];
                echo json_encode($response);
            }
    ...
    public function D_Column()
    {
        header('Content-type: application/json');
        try{
            $DB = new DataBase();
            $nameCatalog = filter_input(INPUT_POST, "NameCatalog");
            $nameDB = filter_input(INPUT_POST, "dataBaseName");
            $column = filter_input(INPUT_POST, "NameColumn");
            $RepositoryName = filter_input(INPUT_POST, "RepositoryName");

            if (isset($nameCatalog) && isset($RepositoryName) && isset($column)){
                $query = "ALTER TABLE $RepositoryName"."_$nameCatalog DROP $column";
                $queryResult = $DB->ConsultaQuery($nameDB, $query);

                if ($queryResult == 1){
                    #$deleteColumn = "Properties###name $NewFieldName###type $FieldType###".$PropertyFieldLength."required $Required###";
                    DesignerForms::DeleteField($nameDB, $RepositoryName."_$nameCatalog", $column);
                    $response = [
                        "status" => true ,
                        "message" => "La columna $column se elimino correctamente."
                    ];
                    echo json_encode($response);
                }
            }else{
                $response = [
                    "status" => false ,
                    "message" => "Algo salio mal al tratar de eliminar la columna. "
                ];
                echo json_encode($response);
            }
